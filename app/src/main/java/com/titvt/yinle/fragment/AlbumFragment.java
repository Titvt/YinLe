...
import com.titvt.yinle.adapter.AlbumRecyclerViewAdapter;
...
public class AlbumFragment extends Fragment {
    ...
    public View onCreateView(@NonNull LayoutInflater inflater, ViewGroup container, Bundle savedInstanceState) {
        FragmentAlbumBinding fragmentAlbumBinding = DataBindingUtil
        ...
        fragmentAlbumBinding.recyclerView.setAdapter(
                new AlbumRecyclerViewAdapter(mainViewModel, getLayoutInflater(), getActivity()));
        fragmentAlbumBinding.setSongInfo(new SongInfo(0, "音乐（lè），享受白嫖的快感",
                0, 0, "", "我再强调一次，第二个字读快乐的乐！"));
        mainViewModel.getCurrentPlaying().observe((LifecycleOwner) viewModelStoreOwner, songDetail ->
                fragmentAlbumBinding.setSongInfo(songDetail.getSongInfo()));
        mainViewModel.getIsPlaying().observe((LifecycleOwner) viewModelStoreOwner, isPlaying ->
                fragmentAlbumBinding.play.setImageResource(!isPlaying ? R.mipmap.play_blue : R.mipmap.pause));
        mainViewModel.getIsShaffle().observe((LifecycleOwner) viewModelStoreOwner, isShaffle ->
                fragmentAlbumBinding.shaffle.setImageResource(isShaffle ? R.mipmap.shaffle : R.mipmap.repeat));
